...
# Get the numbers:
# modified by yangming, 2010.11.26 for display buffers
#my ($free_memory_kb,$used_memory_kb,$caches_kb) = get_memory_info();
my ($free_memory_kb,$used_memory_kb,$cached_kb,$buffers_kb) = get_memory_info();
#print "$free_memory_kb Free\n$used_memory_kb Used\n$caches_kb Cache\n" if ($opt_v);
print "$free_memory_kb Free\n$used_memory_kb Used\n$cached_kb Cache\n$buffers_kb buffers\n" if ($opt_v);

if ($opt_C) { #Do we count caches as free?
#   modified by yangming, 2010.11.26
#    $used_memory_kb -= $caches_kb;
#    $free_memory_kb += $caches_kb;
    $used_memory_kb -= $cached_kb + $buffers_kb;
    $free_memory_kb += $cached_kb + $buffers_kb;
}
...
$cached_kb = sprintf('%d',$cached_kb);
$buffers_kb = sprintf('%d',$buffers_kb);

# Tell Nagios what we came up with
# modified by yangming, 2010.11.26
#tell_nagios($used_memory_kb,$free_memory_kb,$caches_kb);
tell_nagios($used_memory_kb,$free_memory_kb,$cached_kb,$buffers_kb);


sub tell_nagios {
#   modified by yangming, 2010.11.26
#    my ($used,$free,$caches) = @_;
    my ($used,$free,$cached,$buffers) = @_;
...
#    my $perfdata = "|TOTAL=${total}KB;;;; USED=${used}KB;;;; FREE=${free}KB;;;; CACHES=${caches}KB;;;;";
    my $perfdata = "|TOTAL=${total}KB;;;; USED=${used}KB;;;; FREE=${free}KB;;;; CACHED=${cached}KB;;;; BUFFERS=${buffers}KB;;;;";
...
#  print " -C           Count OS caches as FREE memory\n";
  print " -C           Count OS caches(cached+buffers) as FREE memory\n";
...
#   modified by yangming, 2010.11.26 caches_kb=>cached_kb, new buffers_kb
#    my $caches_kb       = 0;
    my $cached_kb       = 0;
    my $buffers_kb	= 0;
...
#		$caches_kb += $2;
                my $counter_name = $1;
                if ($counter_name eq 'Buffers') {
                    $buffers_kb = $2;
                }
                elsif ($counter_name eq 'Cached') {
                    $cached_kb = $2;
                }
            }
        }
        ...
        if ($@) { #Kstat not available
            ...
            my $arc_size = (exists ${kstat}->{zfs} && ${kstat}->{zfs}->{0}->{arcstats}->{size}) ?
                 ...
                 : 0;
#	    modified by yangming, 101126. maybe have bug......
#            $caches_kb += $arc_size;
            $cached_kb += $arc_size;
...
#   return ($free_memory_kb,$used_memory_kb,$caches_kb);
    return ($free_memory_kb,$used_memory_kb,$cached_kb,$buffers_kb);
}
