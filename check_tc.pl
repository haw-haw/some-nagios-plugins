...
# My first Nagios plugin using the Nagios::Plugin modules.  

# Originally by yangming2004 at gmail 
# dot com, July 31 2010
...
my $p = Nagios::Plugin->new(
    ...
    [ -r|--result = <INTEGER> ]",
    version => $VERSION,
    blurb => 'This plugin is an Nagios plugin written in Perl using the Nagios::Plugin modules.  It will alynizing a log file generated by tc service, gathering current connections of service tc and will output OK, WARNING or CRITICAL if the resulting number is outside the specified thresholds.', 
...
my $log = "/tmp/zoomi_tc_monitor";
my $intval = 5 * 60;
unless(open(DATA, $log)) {
	$p->nagios_exit(UNKNOWN, "Can't open $log");
}
my $line;
if ($line = <DATA>) {
	chomp($line);
} else {
	$p->nagios_exit(UNKNOWN, "log is empty");
}
close(DATA);
my $time;
if ( $line =~ /^(\d+)\t(\d+)$/) {
	...
	$p->nagios_exit(UNKNOWN, "data format is wrong");
}
if ((time() - $time) > $intval) {
	$p->nagios_exit(CRITICAL, "log updated $intval seconds before");
} else {
	print " get connection data $result from log file: $log\n " if $p->opts->verbose;
}
...
$p->nagios_exit( 
	 ...
	 message => " current connections was $result" 
);
